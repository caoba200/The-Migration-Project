...
QueueType networkQueue (IO_QUEUE_SIZE);
...
message HALnetworkDriverMessage;
...
processDescriptor networkProcess;
...
volatile sig_atomic_t messageFromHALnetworkDriver = 0; // ---- Assignment 6 -- step 10 --------
...
void Initialize ()
{
    ...
    // ------ Assignment 6 --  Step 10 ---------------
    HALnetworkDriverPid = fork();
    if (HALnetworkDriverPid < 0)
    {
    	cout << "HALos: HALnetwordDriver process creation failed" << endl;
    	exit(1);
    }
    else if (HALnetworkDriverPid == 0)
    {
    	execle ("HALnetworkDriver", "HALnetworkDriver", (char *) NULL, environ);
    	cout << "HALos: HALnetworkDriver failed to load" << endl;
    	exit(1);
    }
...
static void SignalHandler (int signalNo, siginfo_t *info, void *context)
{
    if (signalNo == SIGRTMIN)
    {
        ...
        else if (info -> si_pid == HALnetworkDriverPid)
        {
        	messageFromHALnetworkDriver = 1;
        }
    }
}
